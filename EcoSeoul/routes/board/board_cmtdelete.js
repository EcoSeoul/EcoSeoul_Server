...
router.post('/', async (req, res)=> {
    ...
    if(!cmt_idx|| !user_idx || !board_idx){
        ...
        let deleteQuery = `DELETE FROM Comment WHERE cmt_idx =?  and user_idx = ?`
        let deleteResult = await db.queryParam_Arr(deleteQuery,[cmt_idx, user_idx]);
        let lQuery = `UPDATE Board SET board_cmtnum = board_cmtnum-1 WHERE board_idx =?`;
        let lResult = await db.queryParam_Arr(lQuery,[board_idx]);
        if(!deleteResult || !lResult){
            res.status(500).send({
